...
export async function getAvailableSpotsForDate(date) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      spot:parking_spots(id, label, zone, sort_order),
      released_by_user:profiles!released_by(id, full_name)
    `)
    .eq("date", date)
  ...
  // Resolve active spot owners for recurring spots so overview shows the real owner
  const recurringSpotIds = [...new Set(recurringAvail.map((r) => r.spot_id))]
  let recurringAssignments = []
  if (recurringSpotIds.length > 0) {
    const { data: assigns } = await supabase
      .from("spot_assignments")
      .select("spot_id, user:profiles(id, full_name)")
      .in("spot_id", recurringSpotIds)
      .lte("valid_from", date)
      .or(`valid_until.is.null,valid_until.gte.${date}`)
    recurringAssignments = assigns || []
  }
  const recurringOwnerMap = {}
  recurringAssignments.forEach((a) => {
    if (!recurringOwnerMap[a.spot_id]) recurringOwnerMap[a.spot_id] = []
    recurringOwnerMap[a.spot_id].push(a.user)
  })

  // Get permanently released spots (with their owners via spot_assignments)
  const { data: permanentSpots } = await supabase
    .from("parking_spots")
    .select("id, label, zone, sort_order")
  ...
  const recurringMapped = recurringAvail
    ...
    .map((r) => ({
      ...
      is_recurring: true,
      spot: r.spot,
      released_by_user: (recurringOwnerMap[r.spot_id] || [])[0] || r.owner,
    }))
  ...
  const permanentMapped = (permanentSpots || [])
    ...
    .map((s) => {
      ...
      return {
        ...
        date,
        is_permanent: true,
        spot: { id: s.id, label: s.label, zone: s.zone, sort_order: s.sort_order },
  ...
  // Resolve active parking spot owners for every availability source.
  const availableSpotIds = [...new Set(allAvail.map((a) => a.spot_id))]
  let activeAssignments = []
  if (availableSpotIds.length > 0) {
    const { data: assigns } = await supabase
      .from("spot_assignments")
      .select("spot_id, user:profiles(id, full_name)")
      .in("spot_id", availableSpotIds)
      .lte("valid_from", date)
      .or(`valid_until.is.null,valid_until.gte.${date}`)
    activeAssignments = assigns || []
  }

  const ownerMap = {}
  activeAssignments.forEach((assignment) => {
    if (!ownerMap[assignment.spot_id]) ownerMap[assignment.spot_id] = []
    if (assignment.user) ownerMap[assignment.spot_id].push(assignment.user)
  })

  // Filter out already reserved spots AND owner-blocked spots
  const available = allAvail
    .filter((a) => !reservedSpotIds.has(a.spot_id) && !blockedSpotIds.has(a.spot_id))
    .map((a) => {
      const owners = ownerMap[a.spot_id] || []
      return {
        ...a,
        owners,
        released_by_user: owners[0] || a.released_by_user || null,
      }
    })
