...
function countActiveOwnerApprovals(requiredOwnerIds, approvalOwnerIds) {
  if (!requiredOwnerIds || requiredOwnerIds.size === 0) return 0
  let count = 0
  for (const id of requiredOwnerIds) {
    if (approvalOwnerIds.has(id)) count++
  }
  return count
}

/** Freigabe nur, wenn jeder aktive Inhaber Tages- ODER Wochenrhythmus-Freigabe hat. */
function computeSpotReleaseState({
  spot,
  requiredOwnerIds,
  dayOwnerIds,
  recurringOwnerIds,
  ...
  if (spot.is_permanently_released === true) {
    return {
      fullyReleased: true,
      source: "permanent",
      coveredCount: requiredCount,
      explicitAmongActive: 0,
      recurringAmongActive: 0,
    }
  }
  if (requiredCount === 0) {
    return {
  ...
  let coveredCount = 0
  for (const id of requiredOwnerIds) {
    if (dayOwnerIds.has(id) || recurringOwnerIds.has(id)) coveredCount++
  }

  const explicitAmongActive = countActiveOwnerApprovals(requiredOwnerIds, dayOwnerIds)
  const recurringAmongActive = countActiveOwnerApprovals(requiredOwnerIds, recurringOwnerIds)
  const fullyReleased = coveredCount >= requiredCount

  let source = null
  if (fullyReleased) {
    if (explicitAmongActive >= requiredCount) source = "explicit"
    else if (recurringAmongActive >= requiredCount) source = "recurring"
    else source = "mixed"
  }
  ...
  return {
    ...
    coveredCount,
    explicitAmongActive,
    recurringAmongActive,
  }
}

function mergeDayOwnerMaps(ownerRows, legacyRows, useOwnerTables) {
  const map = buildOwnerSet(ownerRows)
  if (useOwnerTables) return map
  ;(legacyRows || []).forEach((row) => {
    if (!row?.spot_id) return
    if (!map[row.spot_id]) map[row.spot_id] = new Set()
    if (row.released_by) map[row.spot_id].add(row.released_by)
  })
  return map
}

function mergeRecurringOwnerMaps(ownerRows, legacyRows, useOwnerTables) {
  const map = buildOwnerSet(ownerRows)
  if (useOwnerTables) return map
  ;(legacyRows || []).forEach((row) => {
    if (!row?.spot_id) return
    if (!map[row.spot_id]) map[row.spot_id] = new Set()
    if (row.owner_id) map[row.spot_id].add(row.owner_id)
  })
  return map
}
...
export async function removeRecurringAvailability(id) {
  const { data: ownerRow } = await supabase
    .from(OWNER_RECURRING_TABLE)
    .select("id, spot_id, owner_id, weekday")
    .eq("id", id)
    .maybeSingle()
  ...
  const ownerResult = await supabase
    ...
    .eq("id", id)
  if (!isMissingTableError(ownerResult.error)) {
    if (ownerRow?.spot_id && ownerRow?.owner_id && ownerRow?.weekday != null) {
      await supabase
        .from(LEGACY_RECURRING_TABLE)
        .delete()
        .eq("spot_id", ownerRow.spot_id)
        .eq("owner_id", ownerRow.owner_id)
        .eq("weekday", ownerRow.weekday)
    }
    return ownerResult
  }
  ...
  if (!legacyResult.error && legacyRow?.spot_id && legacyRow?.owner_id && legacyRow?.weekday != null) {
    await supabase
      .from(OWNER_RECURRING_TABLE)
      .delete()
      .eq("spot_id", legacyRow.spot_id)
      .eq("owner_id", legacyRow.owner_id)
      .eq("weekday", legacyRow.weekday)
  }
...
export async function getAvailableSpotsForDate(date) {
  ...
  const ownerDayResult = await supabase
    ...
    .eq("date", date)
  const useOwnerDayTables = !isMissingTableError(ownerDayResult.error)
  const ownerDayRows = useOwnerDayTables ? (ownerDayResult.data || []) : []
  const ownerDayError = useOwnerDayTables ? ownerDayResult.error : null
  ...
  let useOwnerRecurringTables = false
  if (weekday >= 1 && weekday <= 5) {
    const ownerRecurringResult = await supabase
      .from(OWNER_RECURRING_TABLE)
      .select("spot_id, owner_id, weekday")
      .eq("weekday", weekday)
    useOwnerRecurringTables = !isMissingTableError(ownerRecurringResult.error)
    if (useOwnerRecurringTables) {
  ...
  const dayOwnerMap = mergeDayOwnerMaps(ownerDayRows, legacyDayRows, useOwnerDayTables)
  const recurringOwnerMap = mergeRecurringOwnerMaps(
    ownerRecurringRows,
    legacyRecurringRows,
    useOwnerRecurringTables
  )
  ...
  const available = (spots || [])
    ...
    .map((spot) => {
      ...
      const requiredCount = requiredOwnerIds.size
      const dayOwnersForSpot = dayOwnerMap[spot.id] || new Set()
      const recurringOwnersForSpot = recurringOwnerMap[spot.id] || new Set()
      const releaseState = computeSpotReleaseState({
        spot,
        requiredOwnerIds,
        dayOwnerIds: dayOwnersForSpot,
        recurringOwnerIds: recurringOwnersForSpot,
      })
      if (!releaseState.fullyReleased) return null
      ...
      return {
        ...
        owner_release_status: {
          approved_count: releaseState.coveredCount,
          required_count: requiredCount,
          is_fully_released: releaseState.fullyReleased,
          source: releaseState.source || "unknown",
        },
      }
    })
    .filter(Boolean)
...
export async function reserveSpot(spotId, availabilityId, userId, date) {
  // Nur legacy-availabilities duerfen als FK in reservations.availability_id landen.
  // IDs aus spot_owner_availabilities (Multi-Owner) oder synthetische IDs muessen null sein.
  let cleanAvailabilityId = null
  const isSyntheticId = (
  ...
  if (availabilityId && !isSyntheticId) {
    const { data: legacyAvailability, error: legacyCheckError } = await supabase
      .from(LEGACY_AVAIL_TABLE)
      .select("id")
      .eq("id", availabilityId)
      .maybeSingle()
    ...
    if (legacyCheckError) {
      return { data: null, error: legacyCheckError }
    }

    cleanAvailabilityId = legacyAvailability?.id || null
  }
...
export async function getDailyOverview(date) {
  ...
  const ownerDayResult = await supabase
    ...
    .eq("date", date)
  const useOwnerDayTables = !isMissingTableError(ownerDayResult.error)
  const ownerDayData = useOwnerDayTables ? (ownerDayResult.data || []) : []
  ...
  if (weekday >= 1 && weekday <= 5) {
    const ownerRecurringResult = await supabase
      ...
      .eq("weekday", weekday)
    useOwnerRecurringTables = !isMissingTableError(ownerRecurringResult.error)
    ownerRecurringData = useOwnerRecurringTables ? (ownerRecurringResult.data || []) : []
  ...
  const explicitOwnerMap = mergeDayOwnerMaps(ownerDayData, legacyDayData, useOwnerDayTables)
  const recurringOwnerMap = mergeRecurringOwnerMaps(
    ownerRecurringData,
    legacyRecurringData,
    useOwnerRecurringTables
  ...
  const overview = (spots || []).map((spot) => {
    ...
    if (spotAssignments.length > 0) {
      ...
      const releaseState = computeSpotReleaseState({
        spot,
        requiredOwnerIds,
        dayOwnerIds: explicitOwnerMap[spot.id] || new Set(),
        recurringOwnerIds: recurringOwnerMap[spot.id] || new Set(),
      })
      const isReleased = releaseState.fullyReleased
      const explicitCount = releaseState.explicitAmongActive
      const recurringCount = releaseState.recurringAmongActive
    ...
    return {
      ...
      ownerReleaseStatus: {
        ...
        covered_count: releaseState.coveredCount,
      },
      reservation,
    }
  })
